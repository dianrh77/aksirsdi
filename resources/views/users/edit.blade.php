        ...
        <div class="flex flex-col xl:flex-row gap-6">
            ...
            <div class="panel flex-1 px-6 py-8">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Edit Data User</h3>
                ...
                <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <select name="positions[]" id="positions" class="form-select w-full" multiple>
                                @foreach ($positionsHierarchy as $pos)
                                    <option value="{{ $pos->id }}"
                                        {{ collect(old('positions', $user->positions->pluck('id')->toArray()))->contains($pos->id) ? 'selected' : '' }}>
                                        {{ $pos->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="role_name" class="form-select w-full" required>
                                ...
                                <option value="kesekretariatan"
                                    {{ old('role_name', $user->role_name) == 'kesekretariatan' ? 'selected' : '' }}>
                                    Kesekretariatan
                                </option>
                                <option value="direktur_utama"
                                    {{ old('role_name', $user->role_name) == 'direktur_utama' ? 'selected' : '' }}>
                                    Direktur Utama
                                </option>
                                <option value="direktur_umum"
                                    {{ old('role_name', $user->role_name) == 'direktur_umum' ? 'selected' : '' }}>
                                    Direktur Umum
                                </option>
                                <option value="user"
                                    {{ old('role_name', $user->role_name) == 'user' ? 'selected' : '' }}>
                                    User
                                </option>
                            </select>
                            @error('role_name')
                    ...
                    <div class="mt-8 flex justify-end">
                        ...
                        <button type="submit" class="btn btn-success  ltr:mr-3">Update</button>
            ...
            <div class="panel w-full xl:w-96">
                <h4 class="text-lg font-semibold mb-4">Panduan Edit User</h4>
                <ul class="list-disc list-inside text-gray-600 dark:text-gray-300 text-sm space-y-2">
                    ...
                    <li>Pastikan role dan status sesuai tugas user.</li>
